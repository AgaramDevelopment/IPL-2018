...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    int daysInMonth = (int)[DateUtil getDaysInMonth:monthToLoad year:yearToLoad];
     ...
     int dates =(int)indexPath.row - firstDay + 1;
    
    int selectEvent = (int)[DateUtil getSelectEventArray :selectEventArray :(monthToLoad+1) year:yearToLoad:dates];
    
    NSDictionary * selectEventColor = [DateUtil getSelectEventColor :selectEventArray :(monthToLoad+1) year:year :dates];
    ...
    if (indexPath.row < firstDay || indexPath.row >= firstDay + daysInMonth) {
            ...
            && yearToLoad == current_year)
        {
            ...
            cell.circleView.layer.borderColor =[currentDateCircleColor CGColor];
            ...
            NSLog(@" index =%ld",(long)indexPath.row);
                ...
                UIColor * selectColor =[self colorWithHexString:[selectEventColor valueForKey:@"backgroundColor"]];//;
                    NSString *ss = [selectEventColor valueForKey:@"backgroundColor"];
                    NSLog(@"%@", ss);
                cell.circleView.backgroundColor = selectColor;
                
                    cell.dateLabel.textColor = selectedDateTextColor;
                    cell.dateLabel.font = [UIFont fontWithName:@"" size:5];
                    }
...
-(void)SetEventTitle:(NSMutableArray *) EventDateArray
{
    NSLog(@"%@",EventDateArray);
    
    selectEventArray =[[NSMutableArray alloc]init];
    selectEventArray =EventDateArray;
